...
avg_rejected_results <- sim_results %>% 
  ...
  geom_point() +
  labs(
    x = "avg_rejected_results")

avg_results + avg_rejected_results
```
From the avg_rejected_results, the sample average mu across tests for which the null is rejected is approximately equal to the true value of mu because when the null is rejected that means the estimate is closer to the true mu, except for lower mu values.